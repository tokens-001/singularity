...
# ── 调度循环的默认并发 ────────────────────────────────────────────────
# 2026-09-21 用户拍板 **2 → 4**。两个理由：
#   · **快**：c 轮执行层实测 54 分钟（并发 2）⇒ 约 27 分钟；
#   · **治"排队等死"**：任务的 900s **从 submit 那刻起算**（排队也算在同一把尺上），
#     并发越小、排得越久 ⇒ 排在后面的任务一次调用都没发起就被砍（c 轮实测 2 个）。
# 🔴 **必须单一来源** —— 以前这个数**有两套**（HTTP 那条默认 1、观察者那两条硬编码 2、
#    启动那次 2），改一处不改另一处 = "改了没生效"，而界面上只会显示一个 `concurrent`。
# ⚠️ 上界是 `web/app.py` 的 `_MAX_CONCURRENT`（8）；别再往代码里写裸数字。
DEFAULT_CONCURRENT = int(os.environ.get("QIDIAN_CONCURRENT", "4"))
