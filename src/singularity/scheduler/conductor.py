...
from singularity.scheduler import dispatcher as _dispatch
from singularity.scheduler import workflow as _wf
        ...
        for step in range(max_steps):
            ...
            if p2 and p2.phase == prev_phase and p2.phase not in (Phase.DONE, Phase.TEMPLATE):
                if not p2.phase.value.startswith("gate"):
                    wait = backoff
                    total_waited = 0
                    while total_waited < 3600:  # 最多等 1 小时
                        if _autopilot_stop.get(project_id, False):
                            break
                        time.sleep(wait)
                        total_waited += wait
                        wait = min(wait * 2, 300)  # 指数退避，上限 5 分钟
