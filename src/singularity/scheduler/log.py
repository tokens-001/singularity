...
def _get_file_logger():
    ...
    if _file_logger is None:
        ...
        _file_logger = logging.getLogger("scheduler.file")
...
def timed(func=None, *, name=""):
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            log_event("fn_start", module=name or f.__name__, fn=f.__name__)
            try:
...
def info(module, msg):
    _get_file_logger().info("%s | %s", module, msg)
    logging.getLogger(module).info(msg)
def warn(module, msg):
    _get_file_logger().warning("%s | %s", module, msg)
    logging.getLogger(module).warning(msg)
def error(module, msg):
    _get_file_logger().error("%s | %s", module, msg)
