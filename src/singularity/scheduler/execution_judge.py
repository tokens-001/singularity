...
5. 顺便生成 test_cases（基于 PRD 成功标准 + API契约 + state_machine）
   ⚠️ **`name` 必须是真实的 pytest 函数名**（形如 `test_normal_path_output_n5`）——
   收尾的集成检查会拿它拼 `pytest -k` 去**真跑**。
   🔴 2026-09-17 真机：那轮 10 条声明的 `name` 全是**中文描述**
   （`parse_ts 时区归一化`、`CLI --level ERROR 正常路径`）⇒ `-k` 一条都选不中
   ⇒ 那 8 个集成用例**压根没跑**，而报告上写着"集成通过"。
