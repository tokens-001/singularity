...
def _expand_review_pool(disp_mod, writer_model: str, chosen: list[str],
    ...
    🔴 **当前恒返回 `[]`（2026-09-18 起，真机确认）** —— 别以为它还活着。
    它挑人问的是 `agent_api_available`，那里面装着"不在**激活池**里就不可用"
    的闸门（`dispatcher.is_model_active`）；而**激活池就是阵容** ⇒ 注册表里
    剩下的候选全被挡住，一个也补不出来。想多人审查得**启用第三个模型**，
    不是指望这里兜底（`test_dispatch_pool_gate.py` 正钉着这条）。

    ⚠️ 别为了让它复活去放宽闸门：那道闸门是给「用户停用的模型被偷偷派去审代码、
    每 20 来秒一轮、真烧钱」（2026-09-18 实锤）加的；放宽的代价是拿 15 个
    **从没启用过**的模型去审代码 —— 同样真花钱。
