...
#      "不同意"里含"同意"⇒ 判批准。⇒ 得管"否定+批准词"这一族。
#   ④ **③的第一版又修过头了**（2026-09-14，逆向审当场打回、我复现属实）：
#      我写的是"批准词**前面**出现过否定就算" —— 于是
#      **「特别同意」**（"别"是构词成分）、**「是否继续？」**（"否"不是否定）都被**误打回**，
#      用户说"我特别同意"、系统把项目**退回上一层**。
#      ⇒ 否定必须**紧挨着**批准词才算，中间最多夹程度词；且**只认「不」**
#      （"别/否"作单字否定极罕见、作构词成分极常见 —— 特别/是否/识别/区别/告别/个别）。
_REJECT_WORDS = ("修改", "改", "不对", "重来", "不通过")

# 否定 + 批准词：**只认紧挨的**，中间允许夹最多两个程度词 / 情态词。
#   ✅ 不同意 · 不通过 · 不确认 · 不继续 · 不太同意 · 不是很同意
#      · **不能通过 · 不想继续 · 不用继续 · 没通过 · 没有通过 · 未通过 · 没必要继续**
#   ❌ 特别同意 · 是否继续 · 这个不错，继续 · 别急，继续 · 差不多同意 · 未来继续
#      —— 这些**都不是否定**，该放行
#
# ⑤ **⑤（2026-09-14 外派⑦ 抓到，我复核属实）**：④ 把否定**收窄成只认「不」**，
#    这一收就漏了**「不」后面跟情态动词**那一族：「**不能通过**」「**不想继续**」
#    「**不用继续**」全被判 **approved** —— 而"不能通过"是高频否定形态。
#    ⇒ 中间那组从"只允许程度词"扩到"程度词 + 情态/助动词"（能想用要会该可敢愿肯）。
#    ⚠️ **`别`/`否` 仍然不收** —— ④ 那次误伤正是它们（特别/是否/识别/区别/告别/个别
#       里的"别/否"是构词成分）。这条边界别动。
#    ⚠️ **同族顺带补上的是 `没`/`未`** —— 「没通过」「没有通过」「未通过」原来也**全判批准**
#       （"没"是高频否定，而我们一个字都没挡）。`没/未` 作构词成分远少于 `别/否`，收得起。
#    ⚠️ 残留（**已知、没修**）：这还是**开集枚举**，换个没列到的说法照样漏
#       （例：「别继续了」—— 落进 `_BARE_REJECT` 要整句相等，这里整句是"别继续了"⇒ 不匹配）。
#       真正的修法是让门用一个**显式的确认动作**、或把判定交给模型，不是在正则上加词。
_NEGATED_APPROVE_RE = re.compile(
    r"(?:不|没|未)(?:太|很|怎么|那么|再|是|能|想|用|要|会|该|可|敢|愿|肯|必|有|法|应|以){0,2}"
    r"(?:通过|继续|确认|同意)")
...
def _is_gate_reply(question: str) -> str | None:
    ...
    **先判打回** —— 见上面 ①②③。
    ...
    if any(w in q for w in _REJECT_WORDS):
        return "rejected"
    # ③④ 否定**紧挨着**批准词才算打回（"不同意"/"不太同意"）。
    #     ⚠️ 不能写成"批准词前面出现过否定"—— 那会把「特别同意」「是否继续？」误打回，
    #     见上面 ④。判据见 `_NEGATED_APPROVE_RE` 的注释。
    if _NEGATED_APPROVE_RE.search(q):
        return "rejected"
    ...
    # ⚠️ `strip` 不是 `rstrip` —— 语气词也会挂**头**上（「**嗯**，可以」「**哦**行」）。
    # 原来只剥尾，「嗯，可以」判不出批准（2026-09-14 顺带修）。方向本来是安全的
    # （判不出 ⇒ None ⇒ 门不推进），但那是白白磨人。
