...
# Estimated price per million tokens (USD)
_MODEL_PRICE_PER_M = {
    # DeepSeek
    ...
    # GLM / Zhipu
    "glm-5-turbo": (0.15, 0.30),
    "glm-5.2": (0.50, 1.00),
    # Kimi
    "kimi-k2.7-code": (0.60, 1.20),
    "kimi-k2.7-code-highspeed": (0.60, 1.20),
    # Qwen
    "qwen3-coder-next": (0.40, 0.80),
    "qwen3-coder-plus": (1.00, 2.00),
    # OpenAI
    "gpt-5.5": (3.00, 12.00),
    "gpt-5.5-pro": (5.00, 20.00),
    # Anthropic
...
def estimate_cost(model: str, input_tokens: int, output_tokens: int) -> float:
    """估算单次调用费用 (USD)。"""
    prices = _MODEL_PRICE_PER_M.get(model)
    if not prices:
        # 尝试模糊匹配
        for k, v in _MODEL_PRICE_PER_M.items():
...
def get_cost_summary(store: ProfileStore = None) -> dict:
    """获取成本汇总：按模型 / 总计。"""
    if store is None:
        store = ProfileStore()
        try:
    ...
    total_cost = 0.0
    total_tokens = 0
    for (model, task_type), stats in store._stats.items():
        est = estimate_cost(model, stats.total_tokens or 0, 0)
        if model not in per_model:
            per_model[model] = {"cost": 0.0, "tokens": 0, "attempts": 0, "successes": 0}
        per_model[model]["cost"] += est
        per_model[model]["tokens"] += stats.total_tokens or 0
        per_model[model]["attempts"] += stats.total_attempts
        per_model[model]["successes"] += stats.successes
        total_cost += est
        total_tokens += stats.total_tokens or 0
    return {
        "total_cost_usd": round(total_cost, 4),
        "total_tokens": total_tokens,
        "per_model": {k: {kk: round(vv, 4) if isinstance(vv, float) else vv
                          for kk, vv in v.items()}
                      for k, v in sorted(per_model.items(), key=lambda x: -x[1]["cost"])},
    }
