...
class RouteResult:
    ...
    `_run_gate()` 跑的是项目自己的 `eval.py`，**没有 `eval.py` 的项目**当时会直接
    `gate失败` ⇒ `retry/rollback`。那是**假失败**，比放行更坏 ——
    拿一个我们不知道的东西去换一个**确定会错**的动作，不划算。

    ⚠️ **09-26 更正**：上面那句"没有 eval.py 的项目会直接 gate失败"**曾经是真的，
    现在不是了** —— `_run_gate()` 加了第三态 `ran`，门**压根没装**时如实披露、不再
    翻成失败（本仓自己就是这种仓：`data/knowledge/scripts/` 下从来没有 `eval.py`）。
    ⇒ 这条反对意见的**依据已经消失**，但**结论不变** —— 兜底 `True` 依然是拿"不知道"
    换一个确定会错的动作。**别因为依据过期就把结论也推翻。**
