...
class PreSearchResult:
    ...
    code_context: str = ""           # codegraph 代码结构上下文
    ...
    try:
        ...
        if file_refs:
            ...
            for fref in file_refs[:3]:
                # 模糊匹配: 文件名可能只是部分路径 (如 "dispatcher.py" vs "scheduler/dispatcher.py")
                match_keys = [k for k in g.get("files", {}) if fref in k or k.endswith(fref)]
                mod_key = fref.replace("/", ".").removesuffix(".py")
                match_mods = [k for k in g.get("import_graph", {}) if mod_key in k or k.endswith(mod_key)]
                ...
                for mk in match_keys[:2]:
                    ...
                    funcs = finfo.get("functions", [])[:5]
                    parts = [f"📁 {mk}"]
                    if classes:
                        parts.append(f"  类: {', '.join(classes)}")
                    if funcs:
                        parts.append(f"  函数: {', '.join(funcs)}")
                    # 被谁 import
                    importers = [k for k, v in g.get("import_graph", {}).items()
                               if any(mk in i or i.endswith(mk.split("/")[-1]) for i in v)]
                    if importers:
                        parts.append(f"  被引用: {', '.join(importers[:5])}")
                    # import 了谁
                    imported = []
                    for mm in match_mods:
                        imported.extend(g.get("import_graph", {}).get(mm, []))
                    if imported:
                        parts.append(f"  引用: {', '.join(imported[:5])}")
                    lines.extend(parts)
            if lines:
                res.code_context = "\n".join(lines)
    except Exception as e:
