...
# 发 `{"action":"chat"}` —— 那是**驱动观察者智能体真调模型**的入口，能烧钱、能读到项目状态。
# ⚠️ **生产路径本来就是对的**：`web/app.py:2289` 显式传了 `host="127.0.0.1"`。所以这条
# 不是"线上正在漏"，是**默认值在往不安全那一侧倒** —— 下一个省略 `host=` 的调用方
# 就会把它暴露出去。翻成回环，要对外暴露得显式写出来。
