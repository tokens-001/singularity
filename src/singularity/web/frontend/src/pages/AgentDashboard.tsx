...
import { Plus, Trash2, Edit3, Wrench, Star, Cpu, Terminal } from 'lucide-react'

const LEVELS = ['D', 'E+', 'E']
const TYPES = [
  { v: 'openai-agent', l: 'API 调用', desc: 'OpenAI 兼容 API，直接调模型' },
  { v: 'claude-cli', l: 'CLI 本地', desc: '本地命令行工具，如 Claude Code/Codex' },
  { v: 'anthropic_api', l: 'Anthropic API', desc: 'Anthropic 原生 API' },
  { v: 'zhipu-api', l: '智谱 API', desc: '智谱专有 API' },
]
...
export default function AgentDashboard() {
  ...
  const handleEdit = (level: string, a: any) => {
    setEditing({level, model: a.model}); setForm({...a, level}); setShowForm(true)
  }
            ...
            <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
              {TYPES.map(t=>(
                <label key={t.v} onClick={()=>setForm({...form,type:t.v})} style={{
                  ...
                  {t.v==='claude-cli'?<Terminal size={14}/>:<Cpu size={14}/>}{t.l}<span style={{fontSize:10,opacity:0.7}}>— {t.desc}</span>
                </label>
              ))}
            </div>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}>
            <div><label style={lbl}>{form.type==='claude-cli'?'工具名':'模型名'}</label><input value={form.model} onChange={e=>setForm({...form,model:e.target.value})} style={inp} placeholder={form.type==='claude-cli'?'codex / claude':'gpt-5.5'}/></div>
            ...
            <div><label style={lbl}>{form.type==='claude-cli'?'CLI 命令':'API 地址'}</label><input value={form.entry||''} onChange={e=>setForm({...form,entry:e.target.value})} style={inp} placeholder={form.type==='claude-cli'?'/path/to/tool -p {prompt}':'https://api.xxx.com/v1'}/></div>
            <div><label style={lbl}>环境变量</label><input value={form.api_key_env||''} onChange={e=>setForm({...form,api_key_env:e.target.value})} style={inp} placeholder="API_KEY_ENV"/></div>
          </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))',gap:10}}>
              {list.map((a:any)=>(
                <div key={a.model} style={{background:'var(--bg-secondary)',border:'1px solid var(--border)',borderRadius:'var(--radius)',padding:14}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'start'}}>
                    <div style={{flex:1}}>
                      <div style={{display:'flex',alignItems:'center',gap:6}}>
                        {isCLI(a) ? <Terminal size={14} color='var(--accent-green)'/> : <Cpu size={14} color='var(--accent)'/>}
                      ...
                      <div style={{fontSize:11,color:'var(--text-secondary)',marginTop:2}}>
                        {isCLI(a) ? <>CLI 工具 · 底层: OpenAI</> : <>{typeLabel(a.type)} · max {a.max_turns||5} turns</>}
                      </div>
                    </div>
                    <div style={{display:'flex',gap:2}}>
                      ...
                      <button onClick={()=>api.deleteAgent(lvl,a.model).then(fetch)} title="禁用" style={{...iconBtn,color:'var(--accent-red)'}}><Trash2 size={14}/></button>
                    </div>
                  </div>
                  ...
                  <div style={{fontSize:10,color:'var(--text-muted)',marginTop:6,fontFamily:'var(--font-mono)',wordBreak:'break-all'}}>{a.entry?.slice(0,80)||'无'}</div>
                </div>
              ))}
            </div>
