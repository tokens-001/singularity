...
if __name__ == "__main__":
    # ⚠️ **必须包在 `__main__` 里**（2026-09-14，外派⑤实测复现）：
    # 这是个**脚本**（用法见文件头），模块级代码会在 import 时直接 `sys.exit(1)`
    # （前置文件 `/tmp/system-architect-test/output.md` 不在时）。平时被 pytest.ini 的
    # `norecursedirs` 挡着没事，但**一旦显式指向它**（`pytest tests/integration/这一份`），
    # import 就触发 SystemExit ⇒ **pytest 整轮 INTERNALERROR**，别的测试也一起跑不成。
    # 它是全 `tests/integration/` 里**唯一**没有这个守卫的文件（其余 20 个都有）。
