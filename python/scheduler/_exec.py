            ...
            for turn in range(1, level_max + 1):
                ...
                if validation.action == "pass" and changed:
                    ...
                    try:
                        ...
                        # Exclude writer model, use 2 different models
                        from . import dispatcher as _d2
                        agents_all = _d2.load_agents()
                        reviewer_models = [a['model'] for a in agents_all.get("D",[])
                                          if a['model'] != writer_model and _d2.agent_api_available(a)][:2]
                        if not reviewer_models:
                            reviewer_models = [a['model'] for a in agents_all.get("E+",[])
                                              if a['model'] != writer_model and _d2.agent_api_available(a)][:2]

                        for changed_file in changed[:5]:  # Review first 5 changed files
                            review = val_mod.multi_model_review(
                                filepath=changed_file, models=reviewer_models, cwd=cwd)
                            if review.get("issues"):
                                ...
                                if crit:
                                    ...
                                    quality["warnings"].append(f"multi-review ({len(review['models_used'])} models) found {len(crit)} critical: {details}")
                            ...
                            quality["quality_signals"]["review_verdicts"] = review.get("verdicts", [])
                            if review.get("verdicts"):
                                needs_fix = [v for v in review["verdicts"] if v.get("verdict") == "needs_fix"]
                                if len(needs_fix) >= 2:  # 2+ models agree it needs fix
                                    validation.action = "retry"
                    except Exception as e:
