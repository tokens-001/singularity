...
用多个便宜模型并行执行 + 合成器交叉比对，替代单个昂贵模型。OpenRouter Fusion 已验证：三个便宜模型（Gemini Flash + Kimi K2.6 + DeepSeek）经合成后得分 64.7%，逼近 Fable 5（65.3%），成本仅一半。Opus 4.8 + GPT-5.5 融合后 67.6%，超越 Fable 5。
...
N 个模型各自产出
  ...
  合成器（裁判模型）交叉比对：
    1. 共识点 → 直接采纳（高置信）
    2. 矛盾点 → 裁判裁决（选更合理的一方）
    3. 遗漏点 → 补全（需求里有但所有人都没提）
    4. 独到见解 → 保留（只有一人提但其他人没覆盖）
       ↓
  一个熔合输出
```
