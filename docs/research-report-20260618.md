...
Singularity当前架构本质上就是Blackboard模式: `ProjectState` 是黑板，D/E/E+层agent通过orchestrator受控读写。
...
Singularity已实现的基础: E→E+→D三级流转 + cost_tier + fallback链。缺失的:
...
Singularity已集成MAGMA核心代码(856行)，但记忆到prompt的注入刚刚接通。下一步:
...
Singularity当前前端是单页HTML+Vanilla JS。差距:
...
Singularity已实现: worktree隔离、`_safe_path`路径检查、危险命令拦截。
...
**Singularity不需要迁移到任何框架。** 原因:

1. Singularity已经是Blackboard架构 — 这是三大框架都做不到的原生模式
2. Singularity的调度器是自己写的，零框架依赖，体积小(dispatcher 330行 vs LangGraph数万行)
3. Singularity的价值在于**自主编排**，不是封装别人的框架
